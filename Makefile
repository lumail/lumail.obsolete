...
LINKER=$(CC) -o
LVER=lua5.1
CPPFLAGS+=-std=gnu++0x -Wall -Werror $(shell pkg-config --cflags ${LVER} glibmm-2.4 gmime-2.6) $(shell pcre-config --cflags) -I/usr/include/ncursesw/
LDLIBS+=$(shell pkg-config --libs ${LVER} glibmm-2.4 gmime-2.6) -lncursesw -lpcrecpp
